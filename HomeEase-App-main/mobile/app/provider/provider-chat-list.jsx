...
export default function ProviderChatList() {
  ...
  const fetchChats = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/provider/chats`, {
  ...
  const renderChat = ({ item }) => {
    const other = (item.participants || []).find(p => p.role !== 'provider');
    return (
      <TouchableOpacity
        style={{ flexDirection: 'row', alignItems: 'center', padding: 16, borderBottomWidth: 1, borderBottomColor: theme.border }}
        onPress={() => router.push({ pathname: '/provider/provider-chat', params: { chatId: item._id } })}
      >
        <Image source={{ uri: other?.image || 'https://via.placeholder.com/40' }} style={{ width: 40, height: 40, borderRadius: 20, marginRight: 12 }} />
        <View style={{ flex: 1 }}>
          <Text style={{ fontWeight: 'bold', color: theme.textDark, fontFamily: Fonts.heading }}>{other?.name || 'Customer'}</Text>
