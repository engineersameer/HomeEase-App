...
let LAST_OLLAMA_VISION_ERROR = null;
let LAST_OLLAMA_VISION_MODEL_USED = OLLAMA_VISION_MODEL;

function getVisionModelCandidates(modelName) {
  const candidates = [modelName];
  if (modelName && !modelName.includes(':')) {
    candidates.push(`${modelName}:latest`);
  }
  return [...new Set(candidates.filter(Boolean))];
}

async function generateOllamaReply(message) {
  try {
    const prompt = `You are HomeEase assistant for home services in Pakistan.
Give practical and concise help in plain text.
User query: ${message}`;
    ...
    console.warn('Ollama unavailable, using fallback AI:', details);
    return null;
  }
}

async function generateOllamaVisionReply({ message, imageBase64 }) {
  const prompt = `You are HomeEase assistant for home services in Pakistan.
You will be shown a photo from a user's home (appliance/area).

Task:
1) Identify what is shown (1 short line)
2) List likely issue(s) (bullets)
3) Give safe, practical next steps (bullets)
4) If the photo is unclear, ask 2-3 specific follow-up questions.

Safety:
- If you suspect electrical/gas hazard, advise to turn off power/gas and call a professional.

User note: ${message || '(no caption provided)'}
Respond in plain text.`;

  const modelCandidates = getVisionModelCandidates(OLLAMA_VISION_MODEL);

  for (const modelName of modelCandidates) {
    LAST_OLLAMA_VISION_MODEL_USED = modelName;

    try {
      // Prefer /api/chat first because some vision models (including moondream variants)
      // are more reliable with chat-style multimodal payloads.
      ...
      const chatReply = (chatResp?.data?.message?.content || '').trim();
      if (chatReply) {
        LAST_OLLAMA_VISION_ERROR = null;
        return chatReply;
      }
    } catch (error) {
      const details = error?.response?.data || error.message;
      LAST_OLLAMA_VISION_ERROR =
        typeof details === 'string' ? details : JSON.stringify(details);
      console.warn(`Ollama vision /api/chat failed for model ${modelName}:`, details);
    }

    try {
      // Fallback to /api/generate for models that still support image generation payloads.
      ...
      const genReply = (genResp?.data?.response || '').trim();
      if (genReply) {
        LAST_OLLAMA_VISION_ERROR = null;
        return genReply;
      }
      ...
      LAST_OLLAMA_VISION_ERROR =
        typeof details === 'string' ? details : JSON.stringify(details);
      console.warn(`Ollama vision /api/generate failed for model ${modelName}:`, details);
    }
  }

  if (!LAST_OLLAMA_VISION_ERROR) {
    LAST_OLLAMA_VISION_ERROR = 'No text response returned by Ollama vision model.';
  }
...
exports.analyzeImage = async (req, res) => {
  try {
    ...
    if (!file.mimetype || !file.mimetype.startsWith('image/')) {
      return res.status(400).json({ success: false, message: 'Unsupported file type. Please upload an image.' });
    }

    const imageBase64 = file.buffer.toString('base64');
    const reply = await generateOllamaVisionReply({ message, imageBase64 });
...
Configured: ${OLLAMA_VISION_MODEL}
Tried: ${LAST_OLLAMA_VISION_MODEL_USED}
Installed in Ollama: ${installed.length ? installed.join(', ') : '(none found)'}
Last Ollama error: ${LAST_OLLAMA_VISION_ERROR || '(none)'}

Fix:
- Wait for download to finish (if pulling)
- Run: ollama pull ${OLLAMA_VISION_MODEL}
- Ensure Ollama is running on ${OLLAMA_BASE_URL}`,
      });
    }
    ...
    return res.json({
      success: true,
      reply,
      sources: [{ category: `Ollama Vision (${OLLAMA_VISION_MODEL})`, confidence: 0.9 }],
    });
  } catch (error) {
...
What home service issue can I help you with today?`;
    ...
    // Prefer local Ollama model when available
    const ollamaReply = await generateOllamaReply(message);
    if (ollamaReply) {
      return res.json({
        success: true,
        reply: ollamaReply,
        sources: [{ category: `Ollama (${OLLAMA_MODEL})`, confidence: 0.95 }]
      });
    }
